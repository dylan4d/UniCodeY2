...
def test():
    ...
    """
    stack = Stack()  #  creates an object or 'instance' of the Stack class
    stack.push(1)
    stack.push(2)
    stack.push(3)
...
def reverse(stack):
    ...
    Returns a new stack, with items in the opposite order.
    """
    result = Stack()
...
def test_reverse_stack():
    """ Test the the reverse function does correctly reverse a stack. 

    No automated checking - need to inspect the displayed output.
    """
...
def infix_to_postfix(string):
    ...
    """
    tokenlist = string.split()  # split returns a list of the substrings, and,
                                # by default, it breaks the string at the spaces
    output = []
    stack = Stack()
...
from stacksLecture import postfix
...
def colour_tetris(stacks, colours, rounds, th, secs):
    ...
    """
    stacklist = []  # a list of stack objects
    for i in range(stacks):
        stacklist.append(Stack())

    charstr = 'RGBOYIV'   # the possible colours for each block

    # generate the list of blocks -- each block is represented by its colour
    blocklist = []
    for i in range(rounds):
    ...
    while i < len(blocklist) and threshold:
        ...
        if elapsed > secs:
            print('TOO LATE (', elapsed, ' sec), so block add to stack 1')
            ans = '1'
        if ans in['1','2','3','4']:
            value = int(ans)-1
        else:
        ...
        if stacklist[value].top() == block:   # successful match
            stacklist[value].pop()            # so remove block from stack
            print(' ******************************** ')
            matches = matches + 1
        else:                          # failed match, so add block to stack
            stacklist[value].push(block)
        if stacklist[value].length() >= th:  # stack is now above height threshold
            threshold = False                # prepare to exit the loop
        else:
            j = 0
            while j < len(stacklist):
                print((j+1), ':', stacklist[j])  # display the current stacks
                j = j+1
