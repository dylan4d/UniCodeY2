...
class Queue(QueueV3):
    """ A Queue, which is a subclass of the class named between "(" and ")"..

    This does not actually add any additional fields or different behaviours
    to the superclass.
    This is a hack so that I can change the Queue implementation without
    changing any of my code that uses a Queue class. To test the behaviour or 
    performance of a specific Queue implementation (e.g. QueueV2 from lectures),
    just change the inheritance above to a different QueueVX? implementation
    e.g. the class declaration above would become   class Queue(QueueV2):
    """
    def __init__(self):
        super().__init__()
